...
`public/data/kpi-from-excel.json` に対して、次の **2 段階**をまとめて（または片方だけ）実行できます。

1. **指標ごとの乱数倍率**（省略可）  
   各 **(スコープ, metric ID)** で actual / forecast / target を **同じ倍率で全月に掛算**（全月で同じ倍率。TvF・TvA のスケール感は保ちやすい）。

2. **月次バンドルのシャッフル**（既定で ON）  
   `metrics["2026-07"]` のような **「1か月分のデータ塊」全体**を、別の月の塊とランダムに入れ替えます。  
   **同じ月の見出しの下では、指標同士の関係はそのまま**ですが、**月をまたぐ推移（スパークラインの傾向）は実態と一致しなくなります**。

```bash
npm run obfuscate-kpi
```

- 上書き時は直前の内容を **`kpi-from-excel.json.orig`** に退避（`.gitignore` でコミットされません）。
- **すでに倍率だけ掛け済み**のファイルに、傾向隠しだけ足す場合:

```bash
npm run obfuscate-kpi:shuffle-only
```

- 倍率レンジ: `--min` / `--max`。再現性: `--seed`（倍率用）、`--shuffle-seed`（月シャッフル用。未指定で `--seed` があるときは `seed+100003`）。

本番の正しい数値が必要なときは、Excel から `npm run export-kpi` で再生成するか、手元の `.orig` から戻してください。
